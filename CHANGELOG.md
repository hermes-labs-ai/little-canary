...
- Codex CLI certification. Codex CLI 0.154.0 loads the existing
  ...
  exercised — `codex plugin add little-canary@hermes-labs` reports
  `installed, enabled` at 0.3.8. Interception itself was **not** observed:
  Codex gates hook execution behind an explicit trust approval, and in a
  headless run an unapproved hook is skipped silently while the prompt proceeds
  to the model. The matrix row stays `runtime_certified: false` and the README
  says so. Codex's schema is vendored under `docs/host-evidence/` and the suite
  asserts every adapter output stays inside it.
- GitHub Copilot certification, as a negative result. Copilot CLI 1.0.84-5
  ...
  rewrite or annotate a prompt but cannot refuse it. Only `preToolUse` carries
  `permissionDecision`, which is an outbound tool control. Copilot also reads a
  different manifest shape (`.github/hooks/*.json`). Little Canary therefore
  ships no Copilot artifact, and a test asserts none appears. The SDK
  declarations are vendored under `docs/host-evidence/`.

### Changed

- The Claude Code marketplace and plugin descriptions name Codex CLI alongside
  Claude Code, matching the install route observed above. Plugin name, version,
  and source path are unchanged, so existing marketplace pins stay valid.
- README gains `Host capability matrix`, `Codex CLI`, and
  `GitHub Copilot CLI` sections.
